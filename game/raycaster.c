...
void    find_side_distances(t_ray *ray , t_game *game)
{
	if (ray->x < 0)
	{
		ray->side_distx = (game->pos_x - (float)game->map_x) * ray->delta_distx;
		ray->step_x = -1;
	}
	else
	{
		ray->side_distx = ((float)game->map_x + 1.0 - game->pos_x) * ray->delta_distx;
		ray->step_x = 1;
	}
	if (ray->y < 0)
	{
	...
	{
		ray->side_disty = ((float)game->map_y + 1 - game->pos_y) * ray->delta_disty;
		ray->step_y = 1;
	}
}
