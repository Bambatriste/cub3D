...
void parse_map(t_game *game, t_params *params)
{
    ...
    i = 0;
    j = 0;
    game->worldMap = ft_calloc(sizeof(char *) , params->map_x_max); //  = 44
    params->testmap = ft_calloc(sizeof(char *) , params->map_x_max);
    while (i < params->map_x_max)
    {
        game->worldMap[i] = ft_calloc(sizeof(char) , params->map_y_max); //  = 34
...
void    copy_line(int i,int len, char *buffer, t_game *game)
{
    ...
    while (j < len)
    {
        ...
        else if (is_player_pos(buffer[j]) && game->params->settings.player_pos == 1)
